...
class Utils:
    ...
    def draw_rectangle(self, image, boxes, color=(0, 255, 0), thickness=3):
        for per_instance in boxes:
            pt1 = (int(per_instance[0]), int(per_instance[1]))
            pt2 = (int(per_instance[2]), int(per_instance[3]))
            cv2.rectangle(image, pt1, pt2, color, thickness)
        return image

    def save_images(self, image, output_dir, name):
        cv2.imwrite(os.path.join(output_dir, f"result_{name}"), image)
